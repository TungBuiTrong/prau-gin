...
class StarGAGE(torch.nn.Module):
    def __init__(self, num_gnn_layers=2, n_classes=5, n_hidden=32, edge_updates=True, edge_dim=63, dropout=0.0, final_dropout=0.5):
        super().__init__()
        ...
        for _ in range(self.num_gnn_layers):
            conv = SAGEConv(n_hidden, n_hidden, aggr='mean')
            self.convs.append(conv)
            self.batch_norms.append(BatchNorm(n_hidden))

        elf.final_mlp = nn.Sequential(
...
for i in range(30):
    model = StarGAGE(edge_dim=63, n_classes=5, n_hidden=64, edge_updates=False, final_dropout=0.25)
    ...
    w_final_metrics['eperiment'] = 'CAT_SAGE'
